...
if [  -f ./raw_report ]; then rm ./final_report; fi
...
if [[ ! -f /last_report || reset ]]; then rm -f last_report max_report; cat new_report | awk '{print 0}' >> last_report; cat last_report >> max_report; fi
